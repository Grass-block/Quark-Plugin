...
public final class QuarkPluginCommand extends CoreCommand {
    ...
    public void onCommand(CommandSender sender, String[] args) {
        switch (args[0]) {
            case "info" -> Quark.sendInfoDisplay(sender);
            case "stats" -> Quark.sendStatsDisplay(sender);
            case "sync-commands" -> {
    ...
    public static final class ReloadCommand extends CoreCommand {
        ...
        public static class ReloadTask implements Runnable {
            ...
            public static void initLoaders() {
                try {
                    PACKAGE_LOADER_CLASS.set(Class.forName("org.tbstcraft.quark.service.framework.PackageManager"));

...
public interface BukkitUtil {
    ...
    static void setBlockUsage(PersistentDataHolder holder) {
        NamespacedKey key = new NamespacedKey(Quark.PLUGIN, "block_usage");
        holder.getPersistentDataContainer().set(key, PersistentDataType.STRING, "elevator");
    }

    static String getBlockUsage(PersistentDataHolder holder) {
        NamespacedKey key = new NamespacedKey(Quark.PLUGIN, "block_usage");
        String s = null;
        try {
    ...
    static void setItemUsage(ItemStack stack, String value) {
        String tag = ChatColor.DARK_GRAY + "%s::%s".formatted("quark", value);
        ItemMeta meta = stack.getItemMeta();
        if (!meta.hasLore()) {
            if (value == null) {
                return;
            ...
            if (value == null) {
                lore.removeIf(s -> s.startsWith(ChatColor.DARK_GRAY + "quark::"));
                meta.setLore(lore);
                stack.setItemMeta(meta);
                return;
            }

            if (Objects.requireNonNull(lore).contains(tag)) {
                return;
            }
            lore.add(tag);
            meta.setLore(lore);
    ...
    @SuppressWarnings("deprecation")
    static boolean checkUsage(ItemStack stack, String value) {
        String tag = ChatColor.DARK_GRAY + "%s::%s".formatted("quark", value);
        ItemMeta meta = stack.getItemMeta();
        if (!meta.hasLore()) {
            return false;
        }
        return Objects.requireNonNull(meta.getLore()).contains(tag);
    }

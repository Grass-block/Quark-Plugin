...
public interface ModuleMeta {
    ...
    static ModuleMeta wrap(AbstractModule m) {
        var lang = Starlight.lang().access(m.getParent().getId());
    ...
    static ModuleMeta create(Class<AbstractModule> reference, IPackage parent, String id) {
        var lang = Starlight.lang().access(parent.getId());
    ...
    String displayName(MinecraftLocale locale);
    ...
    final class AnnotationBasedMeta extends AbstractMeta {
        ...
        private final SLModule descriptor;
        ...
        public AnnotationBasedMeta(Class<AbstractModule> reference, IPackage parent, String namespace, String id, LanguageItem displayName) {
            ...
            this.descriptor = reference.getAnnotation(SLModule.class);
        }
        ...
        public String displayName(MinecraftLocale locale) {
            ...
            if (this.displayName != null && this.displayName.handle().hasAny(this.namespace, "-module-name", this.id)) {
                return "%s&7(&f%s&7)".formatted(displayId.toString(), this.displayName.message(locale));
            }

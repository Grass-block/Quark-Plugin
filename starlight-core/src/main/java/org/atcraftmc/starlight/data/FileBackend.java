    ...
    public void overrideFile(File f, String id) {
        try {
            if (f.getParentFile().mkdirs()) {
                Starlight.instance().getLogger().info("created data folder: " + this.getFolder().getName());
            }

            if (f.createNewFile()) {
                Starlight.instance().getLogger().info("created data file: %s/%s".formatted(this.getFolder().getName(), id));
            }
